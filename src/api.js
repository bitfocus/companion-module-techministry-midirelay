...
module.exports = {
		...
		if (self.config.host) {
			if (self.config.protocol == '3') {
				...
				self.socket.on('midi_back', function (midiObj) {
					if (self.config.midiPort == midiObj.midiport) {
						//only care about the midi port we are listening to
						...
						if (channel == self.config.midiChannel || self.config.ignoreMidiChannels) {
							...
							switch (midiObj.midicommand) {
								...
								case 'cc':
									...
									self.STATUS.lastMidiController =
										self.MIDI_controllers.find(({ id }) => id === midiObj.controller).label || ''
									self.STATUS.lastMidiValue = midiObj.value
									break
								case 'pc':
							...
							if (self.config.useAsSurface) {
								if (midiObj.midicommand == 'noteon' || midiObj.midicommand == 'noteoff') {
									if (
										midiObj.note >= self.config.noteOffset &&
										midiObj.note <= self.config.noteOffset + self.config.maxKeys - 1
									) {
										...
										if (self.config.useAllChannelsAsSurfaces) {
											...
											self.sendCompanionSatelliteCommand(
												`KEY-PRESS DEVICEID=${self.DEVICEID}-ch${midiObj.channel.toString().padStart(2, '0')} KEY=${keyNumber} PRESSED=${keyState}`
											)
										} else {
											self.sendCompanionSatelliteCommand(
												`KEY-PRESS DEVICEID=${self.DEVICEID} KEY=${keyNumber} PRESSED=${keyState}`
											)
										}
									}
								}
	...
	getState: function () {
		//gets the most recent list of midi output ports from midi-relay
		let self = this
