			...
			if (self.config.useAllChannelsAsSurfaces) {
				for (let i = 1; i <= 16; i++) {
					self.sendCompanionSatelliteCommand(
						`REMOVE-DEVICE DEVICEID=${self.DEVICE_ID}-ch${i.toString().padStart(2, '0')}`
					)
				}
			} else {
			...
			for (let index = 0; index < str_split.length; index++) {
				...
				if (command == 'BEGIN') {
					...
					if (self.config.useAllChannelsAsSurfaces) {
						for (let i = 1; i <= 16; i++) {
							self.sendCompanionSatelliteCommand(
								`ADD-DEVICE DEVICEID=${self.DEVICEID}-ch${i.toString().padStart(2, '0')} PRODUCT_NAME="${productName}" KEYS_TOTAL=${self.config.maxKeys} BITMAPS=false COLORS=false TEXT=false`
							)
						}
					} else {
						self.sendCompanionSatelliteCommand(
							`ADD-DEVICE DEVICEID=${self.DEVICEID} PRODUCT_NAME="${productName}" KEYS_TOTAL=${self.config.maxKeys} BITMAPS=false COLORS=false TEXT=false`
						)
					}
					continue
				}
